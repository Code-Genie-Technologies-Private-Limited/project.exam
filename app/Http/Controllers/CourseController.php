...
class CourseController extends Controller
{
    ...
    public function store(StoreCourseRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('courses.index', $request->query())
            ->with('message', 'The Course is Created Successfully');
    }
    ...
    public function update(UpdateCourseRequest $request, Course $course): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('message', 'Course is Updated Successfully');
    }
    ...
    public function destroy(Course $course, Request $request): RedirectResponse
    {
        ...
        if ($course->subCourses()->exists()) {
            return redirect()->route('courses.index', $filters)
                ->with('error', 'Can not delete this course as it has one or more subcourses');
        }

        $course->delete();

        return redirect()->route('courses.index', $filters)
            ->with('message', 'Course is deleted successfully');
    }
}
