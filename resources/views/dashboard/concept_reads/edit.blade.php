                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Concept & read</h4>
                        </div>
                        <div class="card-body">
                            <form method="POST"
                                action="{{ url('/concept-reads/' . $conceptRead->id) . '?' . http_build_query($filters) }}">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description">{{ old('description') ?? $conceptRead->description }}</textarea>
                                        @error('description')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                        ...
                                        <select class="form-control" id="course_id" name="course_id">
                                            @foreach ($courses as $course)
                                                <option value="{{ $course->id }}"
                                                    @if ((old('course_id') ?? $conceptRead->course_id) == $course->id) selected @endif> {{ $course->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('course_id')
                                        ...
                                        <select class="form-control" id="subject_id" name="subject_id">
                                            @foreach ($subjects as $subject)
                                                <option value="{{ $subject->id }}"
                                                    @if ((old('subject_id') ?? $conceptRead->subject_id) == $subject->id) selected @endif> {{ $subject->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('subject_id')
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <input class="form-control" id="order" type="number" name="order"
                                            ...
                                            value="{{ old('order', number_format($conceptRead->order, 2)) }}">
                                        @error('order')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-9 col-form-label">
                                        <div class="form-check form-check-inline mr-1">
                                            <input class="form-check-input" id="active" type="radio" value="1"
                                                name="status" @checked(old('status', $conceptRead->status) == 1)>
                                            <label class="form-check-label" for="active">Active</label>
                                        </div>
                                        <div class="form-check form-check-inline mr-1">
                                            <input class="form-check-input" id="in-active" type="radio" value="0"
                                                name="status" @checked(old('status', $conceptRead->status) == 0)>
