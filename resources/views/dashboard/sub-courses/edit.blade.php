        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ url('/sub-courses/' . $sub-courses->id) . '?' . http_build_query($filters) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-control" id="subject_id" name="subject_id">
                                        ...
                                        <option value="{{ $subject->id }}" @if (( old('subject_id') ?? $subcourse->subject_id) == $subject->id) selected @endif > {{ $subject->name }} </option>
                                        @endforeach
                                    </select>
                                    @error('subject_id')
                                    <span class="text-danger">{{ $message }}</span>
