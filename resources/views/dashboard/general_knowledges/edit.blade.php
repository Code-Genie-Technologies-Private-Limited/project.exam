                        ...
                        <div class="card-body">
                          ...
                          <div class="form-group row">
                             ...
                             <select class="form-control" id="type" name="type">
                              <option value="current affairs" @if ((old('type') ?? $generalKnowledge->type) == 'current affairs') selected @endif>Current Affairs</option>
                              <option value="static" @if ((old('static') ?? $generalKnowledge->type) == 'static') selected @endif>Static</option>
                             </select>
                          </div>
                        </div>
                             <div class="form-group row">
                                        ...
                                        <select class="form-control" id="course_id" name="course">
                                            @foreach ($courses as $course)
                                                <option value="{{ $course->id }}"
                                                    @if ((old('course_id') ?? $generalKnowledge->course_id) == $course->id) selected @endif> {{ $course->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('course_id')
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <input class="form-control" id="order" type="number" name="order"
                                            ...
                                            value="{{ old('order', number_format($question->order, 2)) }}">
                                        @error('order')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-9 col-form-label">
                                        <div class="form-check form-check-inline mr-1">
                                            <input class="form-check-input" id="active" type="radio" value="1"
                                                name="status" @checked(old('status', $question->status) == 1)>
                                            <label class="form-check-label" for="active">Active</label>
                                        </div>
                                        <div class="form-check form-check-inline mr-1">
                                            <input class="form-check-input" id="in-active" type="radio" value="0"
                                                name="status" @checked(old('status', $question->status) == 0)>
