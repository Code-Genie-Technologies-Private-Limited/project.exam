                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ url()->current() }}">
                            <!-- <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" id="name" type="text" name="name" placeholder="Enter courseSubject name" length="160" autocomplete="courseSubject" autofocus value="{{ $filters['name'] ?? '' }}">
                                    @error('name')
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-responsive-sm table-bordered table-striped table-sm mt-2">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $courseSubject->status == 0 ? 'table-danger' : '' }}">
                                    <td>{{ $loop->iteration + ($courseSubjects->currentPage() - 1) * $courseSubjects->perPage() }}</td>
                                    <!-- <td>{{ $courseSubject->name }}
                                        <a href="{{ route('course-subjects.index', ['courseSubject' => $courseSubject->id]) }}">
                                            <span class="badge badge-secondary">{{ $courseSubject->topics_count }}</span>
                                        </a>
                                    </td> -->
                                    ...
                                    <td>
                                        <form action="{{ route('course-subjects.destroy', ['course_subject' => $courseSubject->id]) }}" method="POST">
                                            ...
                                            <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                            @endforeach
                                            <button class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this courseSubject?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
