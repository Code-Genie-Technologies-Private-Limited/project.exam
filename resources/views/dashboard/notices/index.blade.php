                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ url()->current() }}">
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" id="keyword" type="text" name="keyword" placeholder="Enter keyword name" length="160" autocomplete="keyword" autofocus value="{{ ($filters['keyword'] ?? '' )}}">
                                    @error('keyword')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" id="title" type="text" name="title" placeholder="Enter title name" length="160" autocomplete="title" autofocus value="{{ ($filters['title'] ?? '' )}}">
                                    @error('title')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <table class="table table-responsive-sm table-bordered table-striped table-sm mt-2">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $notice->status == 0 ? 'table-danger' : '' }}">
                                    ...
                                    <td>
                                        <a href="{{ url('/notices/' . $notice->id) . '?' . http_build_query(request()->query()) }}" class="btn btn-primary">View</a>
                                    </td>
                                    <td>
                                        <a href="{{ url('/notices/' . $notice->id . '/edit') . '?' . http_build_query(request()->query()) }}" class="btn btn-primary">Edit</a>
                                    </td>
                                    <td>
                                        <form action="{{ route('notices.destroy', ['notice' => $notice->id]) }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            @foreach (request()->query() as $key => $value)
                                            <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                            @endforeach
                                            <button class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this notice?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
