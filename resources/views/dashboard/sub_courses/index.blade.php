                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <input class="form-control" id="name" type="text" name="name"
                                            placeholder="Enter subcourse name" length="160" autocomplete="subcourse"
                                ...
                                <tbody>
                                    @foreach ($subCourses as $subCourse)
                                        <tr class="{{ $subCourse->status == 0 ? 'table-danger' : '' }}">
                                            <td>{{ $loop->iteration + ($subCourses->currentPage() - 1) * $subCourses->perPage() }}
                                            </td>
                                            <td>{{ $subCourse->name }}<span
                                                    class="badge badge-secondary">{{ $subCourse->questions_count }}</span>
                                            </td>
                                            <td class="description">{!! $subCourse->description !!}</td>
                                            <td>{{ $subCourse->course->name }}</td>

                                            <td>{{ $subCourse->order }}</td>
                                            <td>{{ $subCourse->creator->name }}</td>
                                            <td>
                                                <a href="{{ url('/sub-courses/' . $subCourse->id) . '?' . http_build_query(request()->query()) }}"
                                                    class="btn btn-primary">View</a>
                                            </td>
                                            <td>
                                                <a href="{{ url('/sub-courses/' . $subCourse->id . '/edit') . '?' . http_build_query(request()->query()) }}"
                                                    class="btn btn-primary">Edit</a>
                                            </td>
                                            <td>
                                                <form
                                                    action="{{ route('sub-courses.destroy', ['sub_course' => $subCourse->id]) }}"
                                                    method="POST">
                                                    @method('DELETE')
                                                    @csrf
                                                    ...
                                                    <button class="btn btn-danger"
                                                        onclick="return confirm('Are you sure you want to delete this subcourse?')">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
