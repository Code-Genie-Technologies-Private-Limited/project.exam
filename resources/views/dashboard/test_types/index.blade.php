                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    <i class="fa fa-align-left"></i><strong>Test Type List</strong>
                                </div>
                                <div class="col-md-6 text-right">
                                    <a href="{{ route('test-types.create') }}" class="btn btn-primary">Add Test Type</a>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <form method="GET" action="{{ url()->current() }}">
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <input class="form-control @error('name') is-invalid @enderror" id="name"
                                            ...
                                            autocomplete="testtype" autofocus value="{{ $filters['name'] ?? '' }}">
                                        @error('name')
                                ...
                                <a href="{{ route('test-types.index') }}" class="btn btn-primary">Reset</a>
                            </form>
                        </div>
                        <div class="card-body">
                            ...
                            <table class="table table-responsive-sm table-bordered table-striped table-sm mt-2">
                                ...
                                <tbody>
                                    @foreach ($testTypes as $testType)
                                        <tr class="{{ $testType->status == 0 ? 'table-danger' : '' }}">
                                            <td>{{ $loop->iteration + ($testTypes->currentPage() - 1) * $testTypes->perPage() }}
                                            </td>
                                            <td>{{ $testType->name }}<span
                                                    class="badge badge-secondary">{{ $testType->testType_count }}</span>
                                            </td>
                                            <td>{{ $testType->course->name }}</td>
                                            <td>{{ $testType->order }}</td>
                                            <td>{{ $testType->creator->name }}</td>
                                            <td>
                                                <a href="{{ url('/test-types/' . $testType->id) . '?' . http_build_query(request()->query()) }}"
                                                    class="btn btn-primary">View</a>
                                            </td>
                                            <td>
                                                <a href="{{ url('/test-types/' . $testType->id . '/edit') . '?' . http_build_query(request()->query()) }}"
                                                    class="btn btn-primary">Edit</a>
                                            </td>
                                            <td>
                                                <form
                                                    action="{{ route('test-types.destroy', ['test_type' => $testType->id]) }}"
                                                    method="POST">
                                                    @method('DELETE')
                                                    @csrf
                                                    ...
                                                    <button class="btn btn-danger"
                                                        onclick="return confirm('Are you sure you want to delete this test type?')">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                            ...
                            {{ $testTypes->appends($filters)->links() }}
                        </div>
                    </div>
                </div>
            </div>
